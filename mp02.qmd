...
Washington DC had the highest individual income by state at $33,233.
...
baseline <- RENT_BURDEN |>
  ...
  summarise(mean_rent_burden = mean(RENT_BURDEN, na.rm = TRUE),
            mean_index = mean(RENT_BURDEN_IDX, na.rm = TRUE))
  scaled_minmax <- baseline |>
  group_by(year) |>
  mutate(
    min_val = min(RENT_BURDEN, na.rm = TRUE),
    max_val = max(RENT_BURDEN, na.rm = TRUE),
    RENT_BURDEN_MINMAX = 100 * (RENT_BURDEN - min_val) / (max_val - min_val)
  ) |>
  ungroup()
...
Across the country, especially in highly populated areas like NYC, rental costs are sky high. The rental market today is extremely competitive, driving prices higher. This limits families across the nation from affordable, high quality homes. While rental prices have climbed in NIMBY cities like NYC, increases in individual rent have not been sufficient to cover these costs. Students, or recent graduates, are particularly burdened when it comes to high rental costs. 

We will have one respresentative from our YIMBY city Yuma Arizona, and one representative from NYC as sponsors for this bill. 

NYC is a major hub for the arts, education, and city jobs. Modeling NYC's policies to rival one of a NIMBY city may be difficult purely because of the geography of Manhattan, however these policies would benefit local artists, students, and city workers such as MTA employees. Giving these groups access to more affordable, nice housing would allow them to flourish and, in turn, give them a suitable foundation to boost the economy. MTA workers would especially benefit and, since NYC relies on MTA workers to keep the city functioning, this benefit would lead to the running optimally. 
